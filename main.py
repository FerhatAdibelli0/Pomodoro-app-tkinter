...
def count_down(count):
    ...
    if count > 0:
        ...
        for _ in range(math.floor(reps / 2)):
            clicks += "✓"
        checked_label.config(text=clicks)
...
start_button = Button(text="Start", highlightthickness=0, borderwidth=0, command=start_if_condition)
start_button.grid(column=0, row=2)

reset_button = Button(text="Reset", highlightthickness=0, borderwidth=0, command=reset_timer)
reset_button.grid(column=2, row=2)

checked_label = Label(fg=GREEN, bg=YELLOW, font=(FONT_NAME, 30))
checked_label.grid(column=1, row=2)
